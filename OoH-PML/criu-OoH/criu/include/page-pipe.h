...
#define NR_PIPES_PER_CHUNK 64  //bitchebe - may need to increase it from 8 to 32?
...
extern int page_pipe_add_hole(struct page_pipe *pp, unsigned long addr,
			      unsigned int flags);
extern int page_pipe_add_hole_size(struct page_pipe *pp, unsigned long addr, size_t len, unsigned int flags);

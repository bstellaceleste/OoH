...
bool should_dump_page_pml(VmaEntry *vmae)
{
	/*
...
//bitchebe ---> to suppress
int iter;
struct timespec start, end;
double now, time_temp;
//
static int generate_iovs(struct pstree_item *item, struct vma_area *vma, struct page_pipe *pp, u64 *map, u64 *off, bool has_parent, bool pml, unsigned long *list_of_dirty)
{
	u64 *at = NULL;// &map[PAGE_PFN(*off)];
	...
	if( !pml )
	{
		...
		time_temp = ( end.tv_sec - start.tv_sec )*BILLION + ( end.tv_nsec - start.tv_nsec );
		time_Cproc += time_temp;
	}

	nr_to_scan = (vma_area_len(vma) - *off) / PAGE_SIZE;

	//an operation depending on X method --> get_time
	clock_gettime(CLOCK_MONOTONIC, &start);
	if( pml )
	{
		...
		while( i < pml_nb_entries && list_of_dirty[i] != 0 )
		{
			if ( vma_entry_can_be_lazy(vma->e) && !is_stack(item, list_of_dirty[i]) )
				flags |= PPB_LAZY;

			// if ( !should_dump_page_pml(vma->e) )
			// {
			// 	i++;
			// 	continue;
			// }

			//pr_debug("page %lx in vma %lx is being dumped\n", list_of_dirty[i], vma->e->start);
			ret = page_pipe_add_page(pp, list_of_dirty[i], flags);
			//pr_debug("pml_added : %lx - in vma : 0x%lx\n", list_of_dirty[i] / PAGE_SIZE, vma->e->start);
			if (flags & PPB_LAZY && opts.lazy_pages)
				st = 1;
			else
				st = 2;
		...
		time_temp = ( end.tv_sec - start.tv_sec )*BILLION + ( end.tv_nsec - start.tv_nsec );
		time_Cpml += time_temp;
		time_Cpml_pipe_add_page += time_temp;
		//pr_debug("Formula - Cpml_3_%d (pipe_add_page) : %lf ns\n", iter, time_temp);
	...
	for (pfn = 0; pfn < nr_to_scan; pfn++) {
		...
		if (has_parent && page_in_parent(at[pfn] & PME_SOFT_DIRTY)) {
			...
			/* bitchebe: to collect pages added by criu in the dump 
			 * criu logs only modified pages: verified it by also printing vaddr where (at[pfn] & PME_SOFT_DIRTY) is true
			 * and files contents were the same
			 */
			//pr_debug("criu_added : %lx - in vma : %lx\n", vaddr / PAGE_SIZE, vma->e->start);
	...
	time_temp = ( end.tv_sec - start.tv_sec )*BILLION + ( end.tv_nsec - start.tv_nsec );
	time_Cproc += time_temp;
	time_Cproc_pipe_add_page += time_temp;
	//pr_debug("Formula - C/proc_3_%d (pipe_add_page) : %lf ns\n", iter, time_temp);
	
next:
//an operation for Cp --> get_time
	*off += (pml ? nr_to_scan : pfn) * PAGE_SIZE;
                 ...
                 bool pml)
{
	...
	if( !pml )		
	{
		...
		time_temp = ( end.tv_sec - start.tv_sec )*BILLION + ( end.tv_nsec - start.tv_nsec );
		time_Cproc += time_temp;
		time_Cproc_get_vma_map += time_temp;
		//pr_debug("Formula - C/proc_2_%d (get_vma_map) : %lf ns\n", iter, time_temp);
	}
	...
	//{
		//for dirty list
		// uint64_t data, offset, *at_off=&off;
		// int fd_pmap;
		// unsigned int pid = item->pid->real;
		// unsigned long count = 0, addr, nr_to_scan;

		/****************************/
		//collect_pml_logs();
		/****************************/

		// list_dirty = xzalloc(MAX_NB_PFN*sizeof(unsigned long));
		// memset(list_dirty,0,sizeof(MAX_NB_PFN*sizeof(unsigned long)));
		// fd_pmap = open_proc(pid, "pagemap");
		// if(fd_pmap < 0) {
		// 	pr_perror("open pagemap");
		// 	return -ENOMEM;
		// }

		// nr_to_scan = (vma_area_len(vma) - *at_off) / PAGE_SIZE;

		// for (addr = 0; addr < nr_to_scan; addr++) 
		// {
		// 	unsigned long va;

		// 	va = vma->e->start + *at_off + addr * PAGE_SIZE; 
		// 	//let's fill the dirty pages table
		// 	offset = (va / PAGE_SIZE) * sizeof(data);
		// 	if(pread(fd_pmap, &data, sizeof(data), offset) != sizeof(data))
		// 		perror("pread");

		// 	if( (data >> 55) & 1 ){
		// 		list_dirty[count++] = va; //printf("va: %lx\n", va);
		// 		}
		// 	//
		// }
	...
	ret = generate_iovs(item, vma, pp, map, &off, has_parent, pml, list_dirty); 
	if (ret == -EAGAIN) {
        ...
        bool pml)
{
	...
	unsigned long pmc_size, *dirty_app_ad = NULL;
	...
	clock_gettime(CLOCK_MONOTONIC, &start);
	//an operation depending on X method --> get_time
	if( !pml ) //verify that it does not cause crash
	{
		...
		time_temp = ( end.tv_sec - start.tv_sec )*BILLION + ( end.tv_nsec - start.tv_nsec );
		time_Cproc += time_temp;
		//pr_debug("Formula - C/proc_1 : %lf ns\n", time_temp);
	}
	// clock_gettime(CLOCK_MONOTONIC, &start);
	// now = (start.tv_sec * BILLION + start.tv_nsec) - time_temp;
	//pr_debug("Formula - Cp_2 : %lf ns\n", now);
	...
	if( pml && !mdc->pre_dump )
	{
		dirty_app_ad = collect_pml_logs();
		list_dirty = malloc(SIZE);
pr_debug("after collection\n");
		clock_gettime(CLOCK_MONOTONIC, &end);
		time_temp = ( end.tv_sec - start.tv_sec )*BILLION + ( end.tv_nsec - start.tv_nsec );
		time_Cpml += (time_temp - time_RevMap);
		//pr_debug("Formula - Cpml_1 : %lf ns\n", time_temp);
	}

	list_for_each_entry(vma_area, &vma_area_list->h, list) {
		iter++;
		//récupérer de list_dirty les @ qui appartiennent à la vma_i
		//faire que collect renvoie un arr dirty
		//il faut parcourir list_dirty et dès qu'on trouve @1 >= vma_i_start on récupère jusqu'à @2 <= vma_i_end
		//on les met dans list_dirty avant d'appeler generate_vma_iovs		
		...
		{
			unsigned long inc = 0; 
			memset(list_dirty, 0, SIZE);
			//pr_debug("parse_vma_end : %lx\n", vma_area->e->end);
			for( unsigned long var = 0; var < pml_nb_entries && dirty_app_ad[var] != 0; var++ ){
				if( (dirty_app_ad[var] <= vma_area->e->end) ){//pr_debug("parse_vma found : %lx\n", dirty_app_ad[var]);
					while( dirty_app_ad[var] != 0 && dirty_app_ad[var] >= vma_area->e->start && var < pml_nb_entries )
						list_dirty[inc++] = dirty_app_ad[var++];
					break;
				}
			}//printf("inc : %lu\n", inc);
			...
			time_temp = ( end.tv_sec - start.tv_sec )*BILLION + ( end.tv_nsec - start.tv_nsec );
			time_Cpml += time_temp;
			time_Cpml_get_vma_map += time_temp;
			//pr_debug("Formula - Cpml_2_%d (get_vma_map) : %lf ns\n", iter, time_temp);
		}
		
		ret = generate_vma_iovs(item, vma_area, pp, &xfer, args, ctl,
					&pmc, has_parent, mdc->pre_dump,
					parent_predump_mode, pml);
	...
	pr_debug("after loop - xfer_pages : ret = %d\n", ret);
	...
	if( !pml )
	{
		...
		time_temp = ( end.tv_sec - start.tv_sec )*BILLION + ( end.tv_nsec - start.tv_nsec );
		time_Cproc += time_temp;
		time_Cproc_clear_refs += time_temp; 
		//pr_debug("Formula - C/proc_4 (reset_dirty_track) : %lf ns\n", time_temp);
	}
...
out:
	...
	if( !pml )
	{
		...
		time_temp = ( end.tv_sec - start.tv_sec )*BILLION + ( end.tv_nsec - start.tv_nsec );
		time_Cproc += time_temp;
		//pr_debug("Formula - C/proc_5 : %lf ns\n", time_temp);
	}

	pr_info("----------------------------------------\n");
	clock_gettime(CLOCK_MONOTONIC, &b);
	time_impact = ( b.tv_sec - a.tv_sec )*BILLION + ( b.tv_nsec - a.tv_nsec ) - time_Cpml - time_Cproc;
	pr_debug("Formula - time_imp : %lf ns\n", time_impact);
	return exit_code;
}
        ...
        bool pml)
{
	...
	ret = __parasite_dump_pages_seized(item, pargs, vma_area_list, mdc, ctl, pml);
	if (ret) {

...
static int pre_dump_one_task(struct pstree_item *item, InventoryEntry *parent_ie, bool pml)
{
	pid_t pid = item->pid->real;
	...
	ret = parasite_dump_pages_seized(item, &vmas, &mdc, parasite_ctl, pml);
	if (ret)
		goto err_cure;
...
static int dump_one_task(struct pstree_item *item, InventoryEntry *parent_ie, bool pml)
{
	pid_t pid = item->pid->real;
	struct vm_area_list vmas;
	struct parasite_ctl *parasite_ctl;
...
int cr_pre_dump_tasks(pid_t pid)
{
	...
	for_each_pstree_item(item)
		if (pre_dump_one_task(item, parent_ie, opts.pml))
			goto err;
...
int cr_dump_tasks(pid_t pid, bool pml)
{
	InventoryEntry he = INVENTORY_ENTRY__INIT;
	InventoryEntry *parent_ie = NULL;
	struct pstree_item *item;
	...
	pr_debug("Formula - Cp_before_start_frozen : %lf ns\n", time);
	...
	for_each_pstree_item(item) {
		if (dump_one_task(item, parent_ie, pml))
			goto err;
	}

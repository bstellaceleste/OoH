...
static int write_pagemap_to_server(struct page_xfer *xfer, struct iovec *iov, u32 flags, struct page_pipe *pp)
{
...
static int check_pagehole_in_parent(struct page_read *p, struct iovec *iov) //, struct page_pipe *pp)
{
    int ret;
...
static int write_pagemap_loc(struct page_xfer *xfer, struct iovec *iov, u32 flags, struct page_pipe *pp)
{
    int ret;
    ...
    if (flags & PE_PRESENT) {
        ...
        if (xfer->parent != NULL) {
            ret = check_pagehole_in_parent(xfer->parent, iov);//, pp);
            if (ret) {//
                // for(unsigned long ad = (unsigned long)iov->iov_base, i = 0; i < iov->iov_len / PAGE_SIZE; ad += PAGE_SIZE, i++)
                //     page_pipe_add_page(pp, ad, 0);
...
static int page_xfer_dump_hole(struct page_xfer *xfer,
                   struct iovec *hole, u32 flags, struct page_pipe *pp)
{
    ...
    if (xfer->write_pagemap(xfer, hole, flags, pp))
        return -1;
              ...
              unsigned int *cur_hole, void *limit)
{
    ...
    for (; *cur_hole < pp->free_hole ; (*cur_hole)++) {
        ...
        ret = page_xfer_dump_hole(xfer, &hole, hole_flags, pp); 
        pr_debug("dump_holes - ret = %d\n", ret); // bitchebe to remove
        if (ret)
            return -1;
    }
                     ...
                     struct page_pipe *pp)
{
    ...
    list_for_each_entry(ppb, &pp->bufs, l) {
        ...
        for (i = 0; i < aux_len; i++) {
            ...
            if (xfer->write_pagemap(xfer, &iov, flags, pp)) {
                munmap(userbuf, BUFFER_SIZE);
                return -1;
            }
...
int page_xfer_dump_pages(struct page_xfer *xfer, struct page_pipe *pp)
{
    ...
    list_for_each_entry(ppb, &pp->bufs, l) {
        ...
        for (i = 0; i < ppb->nr_segs; i++) {
            ...
            if (xfer->write_pagemap(xfer, &iov, flags, pp)){
                //page_pipe_add_page(pp, (unsigned long)iov.iov_base, 0); //bitchebe -- pml
                return -1;
            }
...
static int page_server_add(int sk, struct page_server_iov *pi, u32 flags)
{
    ...
    psi2iovec(pi, &iov);
    if (lxfer->write_pagemap(lxfer, &iov, flags, NULL))
        return -1;

...
static int generate_iovs(struct pstree_item *item, struct vma_area *vma, struct page_pipe *pp, u64 *map, u64 *off, bool has_parent, bool pml, unsigned long *list_of_dirty)
{
	...
	if( pml )
	{
		...
		while( i < pml_nb_entries && list_of_dirty[i] != 0 )
		{
			...
			/*if ( !should_dump_page(vma->e, 0) )
			{
				pr_debug("page %lx in vma %lx should not be dumped\n", list_of_dirty[i], vma->e->start);
				i++;
				continue;
			}*/

			// if( has_parent && page_in_parent(pfn) )
			// {
			// 	pr_debug("page %lx in vma %lx is in paretn\n", list_of_dirty[i], vma->e->start);
			// 	ret = page_pipe_add_hole(pp, list_of_dirty[i], PP_HOLE_PARENT);							
			// 	st = 0;     
			// }else{
				ret = page_pipe_add_page(pp, list_of_dirty[i], flags);
				pr_debug("pml_added : %lx - in vma : %lx\n", list_of_dirty[i]/PAGE_SIZE, vma->e->start);
        ...
        bool pml)
{
	...
	list_for_each_entry(vma_area, &vma_area_list->h, list) {
		...
		if( pml && !mdc->pre_dump )
		{
			unsigned long inc = 0, var; 
        	memset(list_dirty, 0, SIZE);
			for( var = 0; var < pml_nb_entries && dirty_app_ad[var] != 0; var++ ){
				if( (dirty_app_ad[var] >= vma_area->e->start))// && (dirty_app_ad[var] <= vma_area->e->end) )
				{
					//
					while( (dirty_app_ad[var] >= vma_area->e->start) && (dirty_app_ad[var] <= vma_area->e->end) )//{
						list_dirty[inc++] = dirty_app_ad[var++];
						//pr_debug("list_dirty[%lu] : %lx\n", inc-1, list_dirty[inc-1]);
					//}
				//break;
				}
			}
			pr_debug("pml_list_dirty : %lu\n", var);
		}

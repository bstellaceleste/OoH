...
#   if defined(MPROTECT_VDB) || defined(PROC_VDB) || defined(SOFT_VDB) \
       || defined(USE_MMAP) || defined(PML_VDB)
...
#if (defined(SOFT_VDB) || defined(PML_VDB)) && !defined(NO_SOFT_VDB_LINUX_VER_RUNTIME_CHECK) \
 ...
 *              a fallback strategy.
 * PML_VDB:    We will define a new routine for the use of PML....
...
#if (defined(CHECKSUMS) && (defined(GWW_VDB) || defined(SOFT_VDB) || defined(PML_VDB))) \
    || defined(PROC_VDB)
...
#endif /* CHECKSUMS && (GWW_VDB || SOFT_VDB || PML_VDB) || PROC_VDB */
...
#if defined(PML_VDB)
  static GC_bool pml_init(void);
  //static void pml_disable(void);
#endif
...
#   elif defined(SOFT_VDB) && !defined(PML_VDB)
      if (soft_dirty_init()) {
        GC_COND_LOG_PRINTF("Using soft-dirty bit feature\n");
        return TRUE;
      }
#   elif defined(PML_VDB)
      GC_printf("Using pml-dirty bit feature\n");
      if( pml_init() )
        return TRUE;
#   endif
#   ifdef MSWIN32
...
#if !defined(THREADS) && (defined(PROC_VDB) || defined(SOFT_VDB) || defined(PML_VDB))
...
#if defined(SOFT_VDB) || defined(PML_VDB)
# ifndef VDB_BUF_SZ
...
# endif
  ...
  static GC_bool pml_init(void)
  {
    ...
    snprintf(command, 25, "xl vtf -e PML -p %d", pid);
    ret = system(command);
    ...
    //allocations qui seront initialisées dans collect (pml.h)
    dirty = (unsigned long *)GC_scratch_alloc(SIZE);
    pmap_phys_add = (unsigned long *)GC_scratch_alloc(SIZE); // 
    pmap_virt_add = (unsigned long *)GC_scratch_alloc(SIZE);
...
//#ifdef PML_VDB
  ...
  // {
  //   char command[25];
  //   snprintf(command, 25, "xl vtf -d PML -p %d", saved_proc_pid);
  //   system(command);
  // }
//#endif
...
# else
    GC_INNER GC_bool GC_dirty_init(void)
# endif
  {//GC_printf("bitchebe ----------- do we enter here\n");
...
#   endif
# ifdef PML_VDB
    GC_printf("Using pml-dirty bit feature--2\n");
    if( pml_init() )
      return TRUE;
# endif
    if (!soft_dirty_open_files())
...
//#         ifdef DEBUG_DIRTY_BITS
            //GC_log_printf("soft dirty page at: %p\n", (void *)vaddr);
//#         endif
  ...
  static void parse_maps(void)
  {
    ...
    for (i= 0; i != GC_n_heap_sects; ++i) 
    { 
      start = (unsigned long) GC_heap_sects[i].hs_start;
      end = start + (unsigned long) GC_heap_sects[i].hs_bytes;
      parse_proc_pagemap(saved_proc_pid, start, end);
    }
    for (i = 0; (int)i < n_root_sets; ++i) 
    {
      start = (unsigned long) GC_static_roots[i].r_start;
      end = (unsigned long) GC_static_roots[i].r_end;
      parse_proc_pagemap(saved_proc_pid, start, end);
    }
  }

  GC_INLINE void  GC_pml_read_dirty(GC_bool output_unneeded)
  {
    unsigned long i=0;
    ptr_t vaddr, next_vaddr;
    struct hblk * h;
    CLOCK_TYPE start = CLOCK_TYPE_INITIALIZER, end;
    // memset(pmap_virt_add, 0UL, SIZE);
    // memset(pmap_phys_add, 0UL, SIZE);

    // parse_maps(); //we fill arrays for the reverse mapping
    if( !already_logged )
    {
      GET_TIME(start);
      //BZERO(GC_grungy_pages, sizeof(GC_grungy_pages));
      collect_pml_logs(saved_proc_pid);
      GET_TIME(end);
      GC_printf("Collect and reverse mapping took : %lu ms %lu ns\n #addresses reversed : %lu\n", (unsigned long)MS_TIME_DIFF(end,start), (unsigned long)NS_FRAC_TIME_DIFF(end,start), dirty_nb_entries);
      //pml_disable();
      already_logged = 1;
    }
    ...
    {      
      ...
      i = 0;
      while (i < dirty_nb_entries)
      {
        vaddr = (ptr_t)dirty[i++];
        next_vaddr = vaddr + GC_page_size; 
        for (h = (struct hblk *)vaddr; (word)h < (word)next_vaddr; h++) 
        {
          word index = PHT_HASH(h); 
          set_pht_entry_from_index(GC_grungy_pages, index); 
...
#     ifdef CHECKSUMS
        BZERO(GC_written_pages, sizeof(GC_written_pages));
        GC_or_pages(GC_written_pages, GC_grungy_pages);
#     endif
...
#   elif defined(SOFT_VDB) && !defined(PML_VDB)
//GC_printf("bitchebe -------- in GC_read_dirty -- soft_vdb\n");
      GC_COND_LOG_PRINTF("Using soft-dirty bit feature\n");
      GC_soft_read_dirty(output_unneeded);
#   elif defined(PML_VDB)
//GC_printf("bitchebe -------- in GC_read_dirty -- PML_VDB\n");
      GC_pml_read_dirty(output_unneeded);
...
# if defined(CHECKSUMS) || defined(PROC_VDB)
    ...
    {
#     if defined(GWW_VDB) || defined(PROC_VDB) || defined(SOFT_VDB) || defined(PML_VDB)
        word index;

/////////////////////////////////////////////////////////////////////////
// $Id: vmx.cc 13869 2020-05-29 12:35:30Z sshwarts $
/////////////////////////////////////////////////////////////////////////
//
...
VMX_error_code BX_CPU_C::VMenterLoadCheckVmControls(void)
{
  ...
  if (vm->vmexec_ctrls3 & VMX_VM_EXEC_CTRL3_PML_ENABLE) {
    ...
    /* Here we avoid control on valid address for pml_add as pml may be enable either by the hypv or by the guest -- we'll control it while logging in vmexit.cc */
...
Bit32u BX_CPU_C::VMenterLoadCheckGuestState(Bit64u *qualification)
{
  ...
  *qualification = VMENTER_ERR_NO_ERROR;
  ...
  if (vm->vmcs_linkptr != BX_INVALID_VMCSPTR) {
    ...
    /*
     * Here everything may be ok for the vmcs shadow
     * vtf bitchebe -- Must provide comment later
     */
    vm->guest_pml_address = (bx_phy_address) VMread64_Shadow(VMCS_64BIT_GUEST_PML_ADDRESS);
    if (! IsValidPageAlignedPhyAddr(vm->guest_pml_address))
      vm->guest_pml_address = 0;
    vm->guest_pml_index = VMread16_Shadow(VMCS_16BIT_GUEST_PML_INDEX);
  }
...
void BX_CPU_C::VMexitSaveGuestState(void)
{
  ...
  if (vm->vmexec_ctrls3 & VMX_VM_EXEC_CTRL3_PML_ENABLE) {
    ...
    /*
     * bitchebe -- must provide comment later
     */     
    vm->vmcs_linkptr = VMread64(VMCS_64BIT_GUEST_LINK_POINTER);
    if ( vm->vmcs_linkptr != BX_INVALID_VMCSPTR && IsValidPageAlignedPhyAddr(vm->vmcs_linkptr) && IsValidPageAlignedPhyAddr(vm->guest_pml_address) && vm->guest_pml_address != 0UL )
      if(vm->guest_pml_index <= 511)
      /*
      * I've to verify that the guest_pml_index is in the expected range, because otherwise
      * at the 1st exitsave, the linkpointer has already been allocated but not the index
      * so the value is 111...11 (65535), it will only be 511 at the exit and entry
      */
        VMwrite16_Shadow(VMCS_16BIT_GUEST_PML_INDEX, vm->guest_pml_index);
      else 
        VMwrite16_Shadow(VMCS_16BIT_GUEST_PML_INDEX, 511);
  }
#endif
}
...
#if BX_SUPPORT_VMX >= 2
  if (BX_CPU_THIS_PTR in_vmx_guest){
    //we need to translate the address into a real machine address for effective logging as it is been written by the guest
    if (encoding == VMCS_64BIT_GUEST_PML_ADDRESS){
      val_64 = translate_guest_physical(val_64, 0, 0, 1, BX_RW);
      //BX_INFO(("VMWRITE_GqEq ------------ \n"));
    }
...
void BX_CPU_C::register_vmx_state(bx_param_c *parent)
{
  ...
  * bitchebe -- must provide comment later
  */  
  BXRS_HEX_PARAM_FIELD(vmexec_ctrls, guest_pml_address, BX_CPU_THIS_PTR vmcs.guest_pml_address);
  BXRS_HEX_PARAM_FIELD(vmexec_ctrls, guest_pml_index, BX_CPU_THIS_PTR vmcs.guest_pml_index);

...
{
    if ( !vmx_vcpu_pml_enabled(v) ) //if the guest is using PML do not disable
        return;
...
unsigned long *start_loggint_at(unsigned long *buff)
{
	int prev, curr = prev = 0;
    static unsigned long at[2];

#define PID_FLAG 	0xFFFFFFFF00000000
#define is_flag(x) 	(x & PID_FLAG)==PID_FLAG

	while(is_flag(buff[curr]))
	{
        at[1] = curr + 1; //the idx of the number of adresses previously logged for the last scheduled in process - the hypv will start incrementing at this point
		prev = buff[curr + 1];
		curr += prev + 2; 
	}

    at[0] = curr; //the entry of the buffer where the hypervisor should start logging
...
void vmx_vcpu_flush_pml_buffer(struct vcpu *v)
{
    ...
    unsigned long *shared_pml, *index;
    struct vcpu *v0;
    ...
    if( v->domain->is_vtf )//else don't execute this as it needs for the VM's module to be loaded. Otherwise operations like live migration won't be possible
    {
        ...
        index = start_loggint_at(shared_pml); 
        printk("index[0] %lu - index[1] %lu\n", index[0], index[1]);
    ...
    {                      
        ...
        if( v->domain->enabled_by_vmm ) //if the vmm (dom0 or xen) is using PML
        {    
            /*
        ...
        if( v->domain->is_vtf && v->domain->is_vtf_sched_in)//else don't execute this as it needs for the VM's module to be loaded. Otherwise operations like live migration won't be possible
        {
            /* Save GFNs to the shared memory. */
            spin_lock(&v->domain->vtf_pml_lock);

            // Do nothing if PML buffer is empty. or if the shm has not been reset to 0 by the user
            if (index[0] >= v->domain->vtf_info.nr_pml_entries - 2) //must not be 50 but v->domain->vtf_info.nb_entries - 2(the 2 1st entries used to log the flag and the number of logged ad), just put 50 for tests
                send_guest_vcpu_virq(v0, VIRQ_VTF_PML);
            else{
                printk("log[%lu] : %lu - nb : %lu\n", index[0], shared_pml[index[0]-1], shared_pml[index[1]]);
                shared_pml[index[0]++] = gfn;//pml_buf[pml_idx]; 
                shared_pml[index[1]]++;
            }

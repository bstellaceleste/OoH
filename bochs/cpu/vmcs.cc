...
#if BX_SUPPORT_VMX >= 2
    case VMCS_64BIT_GUEST_PML_ADDRESS:
    case VMCS_64BIT_GUEST_PML_ADDRESS_HI:
      return BX_SUPPORT_VMX_EXTENSION(BX_VMX_PML);
#endif
...
void BX_CPU_C::init_secondary_proc_based_vmexec_ctrls(void)
{
  ...
  //   [15] Reserved (must be '0) //bitchebe: was reserved but it now used for SGX
  ...
  //   [19] Reserved (must be '0) //bitchebe: was reserved but it now used for VMX_TRACE
  //   [20] XSAVES Exiting
  //   [21] Reserved (must be '0)
  //   [22] Mode Based Execution Control (MBE) (not implemented yet)
  //   [23] Sub Page Protection
  //   [24] Reserved (must be '0) //bitchebe: is reserved but I'll use it for vm_proccs

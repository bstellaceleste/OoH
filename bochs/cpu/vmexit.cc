...
void BX_CPU_C::vmx_page_modification_logging(Bit64u guest_paddr, unsigned dirty_update)
{
  ...
  if ( vm->pml_index >= 512 ) {// || vm->guest_pml_index >= 512
  ...
  if (dirty_update) {  
    ...
    //BX_INFO(("vmx_page_modification_logging-----2 \n"));
 ...
 */
void BX_CPU_C::vmx_guest_page_modification_logging(Bit64u guest_laddr, unsigned dirty_update)
{
  VMCS_CACHE *vm = &BX_CPU_THIS_PTR vmcs;
  // if(dirty_update)
  //   BX_INFO(("bitchebelog: index : %lu - addr: %lx", vm->guest_pml_index, guest_laddr ));

  if (vm->guest_pml_index >= 512 ) {// 
    /* Here we should be able to send interrupt to guest */
    Bit8u vector = 64;
 
    if (!is_pending(BX_EVENT_PENDING_VMX_VIRTUAL_INTR))
      {VMX_Self_IPI_Virtualization(vector); //vm->guest_pml_index = 511; 
      //BX_INFO(("bitchebelog: virt_ipi sent"));
      } 
    return;
  }
  
  if (dirty_update && IsValidPageAlignedPhyAddr(vm->guest_pml_address) && vm->guest_pml_address != 0UL) {//dirty_update && 
    Bit64u pLAddr = vm->guest_pml_address + 8 * vm->guest_pml_index;  //ici on va à l'entrée correspondante dans le pml buffer pour écrire la valeur du gla
    access_write_physical(pLAddr, 8, &guest_laddr);
    vm->guest_pml_index--;
    BX_INFO(("bitchebelog: gla: %lx ", guest_laddr));// >> 12
    BX_NOTIFY_PHY_MEMORY_ACCESS(pLAddr, 8, MEMTYPE(resolve_memtype(pLAddr)), BX_WRITE, BX_VMX_PML_WRITE, (Bit8u*)(&guest_laddr));
  }
}
#endif

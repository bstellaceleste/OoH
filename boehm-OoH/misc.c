...
GC_API void GC_CALL GC_enable_incremental() //trying to modifying this function for PML integration
{
  GC_printf("gc_enable_inc ------------11--------- \n");
# if !defined(GC_DISABLE_INCREMENTAL) && !defined(KEEP_BACK_PTRS)
    ...
    if (!GC_find_leak && 0 == GETENV("GC_DISABLE_INCREMENTAL")) {
      LOCK();
      /* Init PML 
      * Enabling it here rather than GC_init because the latter is used in too many other files
      * So to avoid heavy modifications better do it here
      */
      if (!GC_incremental) {
        GC_setpagesize();
        /* if (GC_no_win32_dlls) goto out; Should be win32S test? */
        maybe_install_looping_handler(); /* Before write fault handler! */
        if (!GC_is_initialized) {GC_printf("gc_enable_inc ------------22--------- \n");
          UNLOCK();
          GC_incremental = TRUE; /* indicate intention to turn it on */
          GC_init();
          LOCK();
        } else {GC_printf("gc_enable_inc ------------33--------- \n");
#         if !defined(BASE_ATOMIC_OPS_EMULATED) && !defined(CHECKSUMS) \
...
#         endif
          /* else */ {GC_printf("gc_enable_inc ------------44--------- \n");
            GC_incremental = GC_dirty_init();
          }
        }
      ...
      }GC_printf("gc_enable_inc ------------55--------- \n");
      UNLOCK();GC_printf("gc_enable_inc ------------66--------- \n");
      return;
    }
# endif
  GC_init();

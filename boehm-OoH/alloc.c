...
GC_API int GC_CALL GC_collect_a_little(void)
{
  ...
  //static unsigned complete_collection_time = 0;
...
#   ifndef NO_CLOCK
      if (GC_PRINT_STATS_FLAG) {
        ...
        GET_TIME(current_time);
        time_diff = MS_TIME_DIFF(current_time,start_time);
        complete_collection_time += (1.0*(time_diff*1000) + (NS_FRAC_TIME_DIFF(current_time, start_time)*1.0/1000));
        ...
        GC_log_printf("World-stopped marking took %lu ms %lu ns"
                      " (%u ms in average)\n"
                      "Complete collection since start took %f µs\n",
                      time_diff, NS_FRAC_TIME_DIFF(current_time, start_time),
                      total_time / divisor, complete_collection_time);
      }
#   endif
    return(TRUE);
}

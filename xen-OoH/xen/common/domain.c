...
long do_vcpu_op(int cmd, unsigned int vcpuid, XEN_GUEST_HANDLE_PARAM(void) arg)
{
    ...
    case VCPUOP_vtf_enable_pml:
    {
        ...
        if ( ! atomic_cmpxchg(&d->vtf_pml_flag, 0, 1) )
        {
            ...
            printk("VCPUOP_vtf_enable_pml: PML enabled for domaine %d? : %d\n", d->domain_id,
    ...
    case VCPUOP_vtf_schedule_out:
    {
        vmx_vcpu_flush_pml_buffer(v);
        //if the vmm is using the pml just unset is_vtf flag so that in vmx_vcpu_flush_pml_buffer the hypv does not log in the shd_mem
        if( !d->enabled_by_vmm )
            __vmwrite(GUEST_PML_INDEX, NR_PML_ENTRIES); //setting pml_indx to 512 will cause the cpu to stop logging until the index is reset
        else    
            d->is_vtf_sched_in = 0;
        //printk("VCPUOP_vtf_schedule_out is_vtf %d\n", d->is_vtf);

        break;
    }

    case VCPUOP_vtf_schedule_in:
    {
        if( d->enabled_by_vmm )
            vmx_vcpu_flush_pml_buffer(v); //if the vmm is using the pml aussi, it must collect the addresses currently present in the logs before reseting the index
        //reinitialize the index so that we don't have logs of another process (it does not matter)
        //wether the vmm is using pml or not as the logging process will only restart like after a flush
        __vmwrite(GUEST_PML_INDEX, NR_PML_ENTRIES - 1); 
        d->is_vtf_sched_in = 1;
        //printk("VCPUOP_vtf_schedule_in is_vtf %d\n", d->is_vtf);
    ...
    case VCPUOP_vtf_disable_pml:
    {
        ...
        printk("PML enabled? %d - enabled_by_vmm? %d\n",
                vmx_domain_pml_enabled(d), d->enabled_by_vmm);

        d->is_vtf = 0;
        
        if ( !d->enabled_by_vmm && vmx_domain_pml_enabled(d) &&

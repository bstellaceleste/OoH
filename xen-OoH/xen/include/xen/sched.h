...
struct domain
{
    ...
    bool     is_vtf; //tells the hypervisor how to behave durign pml log flush - if the domain is using ooh, then send the logs
    bool enabled_by_vmm; //tells if the pml was 1st enabled by the dom0 or the hypv itself, so that on VCPUOP_vtf_disble, we dont disable as the hypv may still be using it
    bool     is_vtf_sched_in;
    struct vcpu    **vcpu;

...
int main(int argc, char *argv[], char *envp[])
{
	...
	if (!strcmp(argv[optind], "dump")) {
		...
		double time;
		int dump_res = 0;
		time_Cp = time_Cproc = time_Cpml = time_impact = 0;
		time_Cproc_pipe_add_page = time_Cpml_pipe_add_page = time_Cproc_get_vma_map = time_Cpml_get_vma_map = time_Cproc_clear_refs = 0;
		if (!opts.tree_id)
			goto opt_pid_missing;
		//take the time before and after to get E(C_tker)
		clock_gettime(CLOCK_MONOTONIC, &start);

		dump_res = cr_dump_tasks(opts.tree_id, opts.pml);
		
		clock_gettime(CLOCK_MONOTONIC, &end);

		time = ( end.tv_sec - start.tv_sec )*BILLION + ( end.tv_nsec - start.tv_nsec );
		time_Cp = time - time_Cpml - time_Cproc - time_impact;
		pr_debug("Formula - C_tker : %lf ns\n"
				 "Formula - C_p : %lf\n"
				 "Formula - C_pml : %lf\n"
				 "Formula - C_proc : %lf\n"
				 "Formula - C_RevMap : %lf\n"
				 "Formula - I : %lf\n", time, time_Cp, time_Cpml, time_Cproc, time_RevMap, time_impact);
		pr_debug("Formula - time_Cproc_pipe_add_page : %lf ns\n"
				 "Formula - time_Cpml_pipe_add_page : %lf\n"
				 "Formula - time_Cproc_get_vma_map : %lf\n"
				 "Formula - time_Cpml_get_vma_map : %lf\n"
				 "Formula - time_Cproc_clear_refs : %lf\n", time_Cproc_pipe_add_page, time_Cpml_pipe_add_page, time_Cproc_get_vma_map, time_Cpml_get_vma_map, time_Cproc_clear_refs);
	...
	if (!strcmp(argv[optind], "pre-dump")) {
		if(opts.pml)
		{
			...
			// int ret;
			char command[50];
			snprintf(command, 50, "xl vtf -e PML -p %d", opts.tree_id);
			// clock_gettime( CLOCK_MONOTONIC, &start);
			/* bitchebe - modifying criu to ... */
			ret = system(command);
			// clock_gettime( CLOCK_MONOTONIC, &stop);
			// time = ( stop.tv_sec - start.tv_sec )*BILLION + ( stop.tv_nsec - start.tv_nsec );
			// printf("enable_pml_time : %lf ns\n", time);
			// if( ret < 0 )
			// 	pr_warn("Could not execute the command with system!\n");
		}

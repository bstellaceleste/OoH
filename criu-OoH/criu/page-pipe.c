...
int page_pipe_add_hole_size(struct page_pipe *pp, unsigned long addr, size_t len, unsigned int flags)
{
	...
	//On realloue aussi les flags
		new_size = (pp->nr_holes + PP_HOLES_BATCH) * sizeof(unsigned int);
		if (xrealloc_safe(&pp->hole_flags, new_size))
			return -1;

	//on met aussi à jour nr_holes
		pp->nr_holes += PP_HOLES_BATCH;
	}

	new_iov = &pp->holes[pp->free_hole++];
	new_iov->iov_base = (void*)addr;
	//taille en octets
	new_iov->iov_len = len * PAGE_SIZE;

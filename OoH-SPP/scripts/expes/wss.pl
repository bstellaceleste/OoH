...
my $clear_ref = "/proc/$cmd/clear_refs";
my $smaps = "/proc/$cmd/smaps";
...
if ($profile) {
	...
	if ($snapshot == 0) {
		printf "Watching PID $cmd page references for every $duration seconds...\n";
	} else {
		printf "Watching PID $cmd page references for $duration seconds, repeating after $snapshot second pauses...\n";
	}
} else {

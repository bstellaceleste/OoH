...
static void spp_handle(struct p2m_domain *p2m, paddr_t gpa)
{
    ...
    if (p2m->spp_get_entry(p2m, gaddr_to_gfn(gpa), &bitmap))
    {
        ...
        {
            /* granted access behavior */
            /* hardware signal step */
            //printk("SPP: spp_handle(): ACCESS_GRANTED\n");
            struct cpu_user_regs *regs = guest_cpu_user_regs();
        ...
        else
        {
            ...
            printk("SPP: spp_handle(): ACCESS_DENIED %x, num subpage %ld\n", gaddr_to_gfn(gpa), subpage);
            hvm_inject_hw_exception(TRAP_gp_fault, 0);
        }
    }
}
...
static void ept_handle_violation(ept_qual_t q, paddr_t gpa)
{
    ...
    case HVM_SPP_WRITE_PROTECTED: {
        ...
        spp_handle(p, gpa); // <== single step //hvm_inject_hw_exception(TRAP_gp_fault, 0); // 
        return;
    }

...
struct xen_hvm_spp_trash {
	uint64_t trash[SPP_TRASH_SZ];
	int size;
 	int zeroed:1;
};
typedef struct xen_hvm_spp_trash xen_hvm_spp_trash_t;
DEFINE_XEN_GUEST_HANDLE(xen_hvm_spp_trash_t);
...
long do_hvm_op(unsigned long op, XEN_GUEST_HANDLE_PARAM(void) arg)
{
    ...
    case HVMOP_release_subpage: {
        //printk("SPP: release subpage batching mode\n");
        xen_hvm_spp_trash_t var;
        struct domain *d;
        int i;
        
        if ( copy_from_guest(&var, arg, 1 ) )
             return -EFAULT;
        ...
        for (i = 0; i < var.size; i++) {
            ...
            rc = p2m_release_subpage(d, var.trash[i]);
        } 
    ...
    case HVMOP_set_bitmap: {
        xen_hvm_spp_bitmap_t var;
        int idx = 0;
        struct domain *d;
    
        memset(var.tab, 0, SPP_PAGES_MAX*sizeof(uint64_t));

         if ( copy_from_guest(&var, arg, 1 ) ){
            printk("SPP: Not_range ERROR!!!!!!!!!\n");
             return -EFAULT;
        }
        ...
        //printk("SPP: nb_pages %d bitmap %x\n", var.nb_pages, var.bitmap);        
        for (i = 0; (i < var.nb_pages); i++) {
            if (var.tab[i] == 0) {
                //printk("--------- SPP set bitmap: pab pfn ---------\n");
                continue;
            }
            //printk("SPP: gfn %lx\n", var.tab[i]);
            rc = p2m_set_bitmap(d, var.tab[i], var.bitmap);
        }
        
        //printk("SPP: not_range - after\n");
        rcu_unlock_domain(d);
        break; 
    }

    case HVMOP_set_bitmap_range: {
        xen_hvm_spp_bitmap_range_t var;
        int idx = 0;
        struct domain *d;
        ...
        //printk("SPP: range - nb_pages %d bitmap %x start %x\n", var.nb_pages, var.bitmap, var.range_start);
        rc = p2m_set_range_bitmap(d, var.range_start, var.bitmap, var.nb_pages);
        //printk("SPP: range - after\n");
        
        rcu_unlock_domain(d);
        break; 
    }

    case HVMOP_reset_subpage: {
        xen_hvm_subpage_t subpage;
        struct domain *d;
        //printk("reset gfn %lx\n", spp_gfn);

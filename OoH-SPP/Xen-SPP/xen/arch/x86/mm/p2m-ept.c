...
static int
ept_spp_update_range_wp(struct p2m_domain *p2m, unsigned long gfn, int size)
{
    ...
    int ret, rc;
    unsigned long gfn_origin, gfn_tmp;
...
while(idx < 512 && count) {
    gfn_tmp = gfn_origin + idx;
    ept_entry = table + (idx >> (i * EPT_TABLE_ORDER));
    ...
    //printk("spp: ept_spp_update_range_wp gfn %lx\n", gfn_origin);
...
{
    //printk("spp_access_check: bitmap %lx\n", bitmap);
    ...
    __u64 subpage = offset >> 7; //depuis les 12 précédents on se déplace de 7 pour garder 5

    /* on veut le bit qui correspond à la subpage */
    __u64 access = bitmap >> 2*subpage;
    //printk("SPP: gpa %x, subpage %d, access %x, access subpage %u\n", gpa, subpage, access, access & 0x1);
    return access & 0x1;
}
...
static int
bitmap_spp_range_set_entry(struct p2m_domain *p2m, unsigned long gfn, u64 access, int size)
{
    struct spp_data *spp = &p2m->spptp;
    unsigned long gfn_remainder = gfn;
    spp_entry_t *table;
    ...
    pspp_bitmap = (u64 *) (table + idx);
    write_atomic(pspp_bitmap, access);


    while (idx < 512 && remain) {
        pspp_bitmap = (u64 *) (table + idx);
        write_atomic(pspp_bitmap, access);
        idx++;
        remain--;
...
static int
bitmap_spp_set_entry(struct p2m_domain *p2m, unsigned long gfn, u64 access)
{
    struct spp_data *spp = &p2m->spptp;
    unsigned long gfn_remainder = gfn;
    spp_entry_t *table;
...
int ept_p2m_init(struct p2m_domain *p2m)
{
    ...
    p2m->update_ept_spp_range_wp = ept_spp_update_range_wp;
p2m->bitmap_spp_range_set_entry = bitmap_spp_range_set_entry;

   p2m->update_ept_spp_wp = ept_spp_update_wp;
   p2m->bitmap_spp_set_entry = bitmap_spp_set_entry;

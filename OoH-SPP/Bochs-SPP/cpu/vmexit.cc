...
void BX_CPU_C::vmx_page_modification_logging(Bit64u guest_paddr, Bit64u guest_laddr, unsigned dirty_update)
{
  VMCS_CACHE *vm = &BX_CPU_THIS_PTR vmcs;
  Bit64u cr3_curr = BX_CPU_THIS_PTR cr3;
  Bit64u cr3_tracked;

  if (vm->pml_index >= 512) { //512
    Bit32u vmexit_qualification = 0;
    if (BX_CPU_THIS_PTR nmi_unblocking_iret)
      vmexit_qualification |= (1 << 12);
    
    //BX_INFO(("BX_CPU_C::vmx_page_modification_logging"));
    VMexit(VMX_VMEXIT_PML_LOGFULL, vmexit_qualification);
  }

  if (dirty_update) {
    Bit64u pAddr = vm->pml_address + 8 * vm->pml_index;  //ici on va à l'entrée correspondante dans le pml buffer pour écrire la valeur du gpa
     ...
     */
    if(IsValidPageAlignedPhyAddr(vm->guest_pml_address) && vm->guest_pml_address != 0UL){
      Bit64u pLAddr= vm->guest_pml_address + 8;
      access_read_physical(pLAddr, 8, &cr3_tracked);
      //if(cr3_tracked != 0 && (cr3_curr - cr3_tracked) < 10UL ){// vm->pml_index must no be 0 so that we should not modify the first entry of the log which corresponds to the saved cr3_tracked
        if(vm->pml_index > 1){
          pLAddr = vm->guest_pml_address + 8 * vm->pml_index;
          access_write_physical(pLAddr, 8, &guest_laddr);
          BX_INFO(("before_bitchebe: gla = %lu - cr3_curr = %lu - cr3_tracked = %lu - diff = %lu\n", guest_laddr >> 12, cr3_curr, cr3_tracked, (cr3_curr - cr3_tracked)));
          BX_NOTIFY_PHY_MEMORY_ACCESS(pLAddr, 8, MEMTYPE(resolve_memtype(pLAddr)), BX_WRITE, BX_VMX_GUEST_PML_WRITE, (Bit8u*)(&guest_laddr));
        //}
        //BX_INFO(("after_bitchebe: gla_after = %lu - cr3_curr = %ld\n", guest_laddr, cr3_curr));
      }
    }

/////////////////////////////////////////////////////////////////////////
// $Id: access2.cc 13699 2019-12-20 07:42:07Z sshwarts $
/////////////////////////////////////////////////////////////////////////
//
    ...
    BX_CPU_C::write_linear_byte(unsigned s, bx_address laddr, Bit8u data)
{
  ...
  if (tlbEntry->lpf == lpf)
  {
    // BX_INFO(("PERF: TLB HIT NORMAL %lx\n", laddr));
    ...
    if (isSPPOK(tlbEntry))
    {
      int start = (laddr & 0xfff) >> 7;
      BX_INFO(("byte: tlbEntry ppf 0x" FMT_PHY_ADDRX " bitmap " FMT_PHY_ADDRX " laddr 0x" FMT_PHY_ADDRX " subpage %d\n", tlbEntry->ppf, tlbEntry->spp_bitmap, laddr, start));
      if ((tlbEntry->spp_bitmap >> 2 * start))
      {
        BX_INFO(("PERF: TLB HIT SPP %lx\n", laddr));
        bx_hostpageaddr_t hostPageAddr = tlbEntry->hostPageAddr;
        Bit32u pageOffset = PAGE_OFFSET(laddr);
        bx_phy_address pAddr = tlbEntry->ppf | pageOffset;
        BX_NOTIFY_LIN_MEMORY_ACCESS(laddr, pAddr, 1, tlbEntry->get_memtype(), BX_WRITE, (Bit8u *)&data);
      ...
      {
        BX_INFO((" byte: access denied laddr %lx %lx bitmap %lx\n", laddr, tlbEntry->ppf, tlbEntry->spp_bitmap));
        exception(int_number(s), 0);
        return;
      }
    }
  }
  // BX_INFO(("PERF: TLB MISS NORMAL %lx\n", laddr));
    ...
    BX_CPU_C::write_linear_word(unsigned s, bx_address laddr, Bit16u data)
{
  ...
  if (tlbEntry->lpf == lpf)
  {
    ...
    {
      int start = (laddr & 0xfff) >> 7;
      bx_address laddr_end = laddr + 1;
      BX_INFO(("word: tlbEntry ppf 0x" FMT_PHY_ADDRX " bitmap " FMT_PHY_ADDRX " laddr 0x" FMT_PHY_ADDRX " subpage %d\n", tlbEntry->ppf, tlbEntry->spp_bitmap, laddr, start));
      if (((tlbEntry->spp_bitmap >> 2 * start) & 1))
      {
        ////BX_INFO(("spp: access granted\n"));
        bx_hostpageaddr_t hostPageAddr = tlbEntry->hostPageAddr;
        Bit32u pageOffset = PAGE_OFFSET(laddr);
        bx_phy_address pAddr = tlbEntry->ppf | pageOffset;
        BX_NOTIFY_LIN_MEMORY_ACCESS(laddr, pAddr, 2, tlbEntry->get_memtype(), BX_WRITE, (Bit8u *)&data);
      ...
      {
        BX_INFO((" word: access denied laddr %lx %lx bitmap %lx\n", laddr, tlbEntry->ppf, tlbEntry->spp_bitmap));
        exception(int_number(s), 0);
        return;
      }
    }
    ...
    BX_CPU_C::write_linear_dword(unsigned s, bx_address laddr, Bit32u data)
{
  ...
  if (tlbEntry->lpf == lpf)
  {
    ...
    {
      int start = (laddr & 0xfff) >> 7;
      bx_address laddr_end = laddr + 3;
      BX_INFO(("dword: tlbEntry ppf 0x" FMT_PHY_ADDRX " bitmap " FMT_PHY_ADDRX " laddr 0x" FMT_PHY_ADDRX " subpage %d\n", tlbEntry->ppf, tlbEntry->spp_bitmap, laddr, start));
      if (((tlbEntry->spp_bitmap >> 2 * start) & 1))
      {
        ////BX_INFO(("spp: access granted\n"));
        bx_hostpageaddr_t hostPageAddr = tlbEntry->hostPageAddr;
      ...
      {
        BX_INFO((" dword: access denied laddr %lx %lx bitmap %lx\n", laddr, tlbEntry->ppf, tlbEntry->spp_bitmap));
        exception(int_number(s), 0);
        return;
      }
    }
    ...
    BX_CPU_C::write_linear_qword(unsigned s, bx_address laddr, Bit64u data)
{
  ...
  if (tlbEntry->lpf == lpf)
  {
    ...
    if (isSPPOK(tlbEntry))
    {
      ...
      int start = (laddr & 0xfff) >> 7;
      bx_address laddr_end = laddr + 7;
      BX_INFO(("qword: tlbEntry ppf 0x" FMT_PHY_ADDRX " bitmap " FMT_PHY_ADDRX " laddr 0x" FMT_PHY_ADDRX " subpage %d\n", tlbEntry->ppf, tlbEntry->spp_bitmap, laddr, start));
      if (((tlbEntry->spp_bitmap >> 2 * start) & 1))
      {
        ////BX_INFO(("spp: access granted\n"));
        bx_hostpageaddr_t hostPageAddr = tlbEntry->hostPageAddr;
      ...
      {
        BX_INFO(("qword: access denied pfn %lx bitmap %lx\n", tlbEntry->ppf, tlbEntry->spp_bitmap));
        exception(int_number(s), 0);
        return;
      }
    }
    ...
    BX_CPU_C::write_linear_xmmword(unsigned s, bx_address laddr, const BxPackedXmmRegister *data)
{
  ...
  if (tlbEntry->lpf == lpf)
  {
    ...
    if (isSPPOK(tlbEntry))
    {
      ...
      int start = (laddr & 0xfff) >> 7;
      bx_address laddr_end = laddr + 15;
      BX_INFO(("xmmword: tlbEntry ppf 0x" FMT_PHY_ADDRX " bitmap " FMT_PHY_ADDRX " laddr 0x" FMT_PHY_ADDRX " subpage %d\n", tlbEntry->ppf, tlbEntry->spp_bitmap, laddr, start));
      if (((tlbEntry->spp_bitmap >> 2 * start) & 1))
      {
        ////BX_INFO(("spp: access granted\n"));
        bx_hostpageaddr_t hostPageAddr = tlbEntry->hostPageAddr;
      ...
      {
        BX_INFO(("xmmword: access denied laddr %lx %lx bitmap %lx\n", laddr, tlbEntry->ppf, tlbEntry->spp_bitmap));
        exception(int_number(s), 0);
        return;
      }
    }
    ...
    BX_CPU_C::write_linear_xmmword_aligned(unsigned s, bx_address laddr, const BxPackedXmmRegister *data)
{
  ...
  if (tlbEntry->lpf == lpf)
  {
    ...
    if (isSPPOK(tlbEntry))
    {
      int start = (laddr & 0xfff) >> 7;
      bx_address laddr_end = laddr + 15;
      BX_INFO(("xmmword_aligned: tlbEntry ppf 0x" FMT_PHY_ADDRX " bitmap " FMT_PHY_ADDRX " laddr 0x" FMT_PHY_ADDRX " subpage %d\n", tlbEntry->ppf, tlbEntry->spp_bitmap, laddr, start));
      if (((tlbEntry->spp_bitmap >> 2 * start) & 1))
      {
        ////BX_INFO(("spp: access granted\n"));
        bx_hostpageaddr_t hostPageAddr = tlbEntry->hostPageAddr;
      ...
      {
        BX_INFO(("xmmword_aligned: access denied laddr %lx %lx bitmap %lx\n", laddr, tlbEntry->ppf, tlbEntry->spp_bitmap));
        exception(int_number(s), 0);
        return;
      }
    }
  ...
  if (laddr & 15)
  {
    BX_ERROR((" xmmword_aligned(): #GP misaligned access"));
    ...
    BX_CPU_C::write_linear_ymmword(unsigned s, bx_address laddr, const BxPackedYmmRegister *data)
{
  ...
  if (tlbEntry->lpf == lpf)
  {
    ...
    {
      int start = (laddr & 0xfff) >> 7;
      bx_address laddr_end = laddr + 31;
      BX_INFO(("ymmword: tlbEntry ppf 0x" FMT_PHY_ADDRX " bitmap " FMT_PHY_ADDRX " laddr 0x" FMT_PHY_ADDRX " subpage %d\n", tlbEntry->ppf, tlbEntry->spp_bitmap, laddr, start));
      if (((tlbEntry->spp_bitmap >> 2 * start) & 1))
      {
        // BX_INFO(("spp: access granted\n"));
        bx_hostpageaddr_t hostPageAddr = tlbEntry->hostPageAddr;
      ...
      {
        BX_INFO(("ymmword: access denied laddr %lx %lx bitmap %lx\n", laddr, tlbEntry->ppf, tlbEntry->spp_bitmap));
        exception(int_number(s), 0);
        return;
      }
    }
    ...
    BX_CPU_C::write_linear_ymmword_aligned(unsigned s, bx_address laddr, const BxPackedYmmRegister *data)
{
  ...
  if (tlbEntry->lpf == lpf)
  {
    ...
    if (isSPPOK(tlbEntry))
    {
      int start = (laddr & 0xfff) >> 7;
      bx_address laddr_end = laddr + 31;
      BX_INFO(("ymmword_aligned: tlbEntry ppf 0x" FMT_PHY_ADDRX " bitmap " FMT_PHY_ADDRX " laddr 0x" FMT_PHY_ADDRX " subpage %d\n", tlbEntry->ppf, tlbEntry->spp_bitmap, laddr, start));
      if (((tlbEntry->spp_bitmap >> 2 * start) & 1))
      {
        ////BX_INFO(("spp: access granted\n"));
        bx_hostpageaddr_t hostPageAddr = tlbEntry->hostPageAddr;
      ...
      {
        BX_INFO(("ymmword_aligned: access denied laddr %lx %lx bitmap %lx\n", laddr, tlbEntry->ppf, tlbEntry->spp_bitmap));
        exception(int_number(s), 0);
        return;
      }
    }
  ...
  if (laddr & 31)
  {
    BX_ERROR((" ymmword_aligned(): #GP misaligned access"));
    exception(BX_GP_EXCEPTION, 0);
  }
  // BX_INFO(("PERF: TLB MISS NORMAL %lx\n", laddr));
    ...
    BX_CPU_C::write_linear_zmmword(unsigned s, bx_address laddr, const BxPackedZmmRegister *data)
{
  ...
  if (tlbEntry->lpf == lpf)
  {
    ...
    {
      int start = (laddr & 0xfff) >> 7;
      bx_address laddr_end = laddr + 63;
      BX_INFO(("zmmword: tlbEntry ppf 0x" FMT_PHY_ADDRX " bitmap " FMT_PHY_ADDRX " laddr 0x" FMT_PHY_ADDRX " subpage %d\n", tlbEntry->ppf, tlbEntry->spp_bitmap, laddr, start));
      if (((tlbEntry->spp_bitmap >> 2 * start) & 1))
      {
        ////BX_INFO(("spp: access granted\n"));
        bx_hostpageaddr_t hostPageAddr = tlbEntry->hostPageAddr;
      ...
      {
        BX_INFO(("zmmword: access denied laddr %lx %lx bitmap %lx\n", laddr, tlbEntry->ppf, tlbEntry->spp_bitmap));
        exception(int_number(s), 0);
        return;
      }
    }
    ...
    BX_CPU_C::write_linear_zmmword_aligned(unsigned s, bx_address laddr, const BxPackedZmmRegister *data)
{
  ...
  if (tlbEntry->lpf == lpf)
  {
    ...
    if (isSPPOK(tlbEntry))
    {
      int start = (laddr & 0xfff) >> 7;
      bx_address laddr_end = laddr + 63;
      BX_INFO(("zmmword_aligned: tlbEntry ppf 0x" FMT_PHY_ADDRX " bitmap " FMT_PHY_ADDRX " laddr 0x" FMT_PHY_ADDRX " subpage %d\n", tlbEntry->ppf, tlbEntry->spp_bitmap, laddr, start));
      if (((tlbEntry->spp_bitmap >> 2 * start) & 1))
      {
        ////BX_INFO(("spp: access granted\n"));
        bx_hostpageaddr_t hostPageAddr = tlbEntry->hostPageAddr;
      ...
      {
        BX_INFO(("zmmword_aligned: access denied laddr %lx %lx bitmap %lx\n", laddr, tlbEntry->ppf, tlbEntry->spp_bitmap));
        exception(int_number(s), 0);
        return;
      }
    }
  ...
  if (laddr & 63)
  {
    BX_ERROR((" zmmword_aligned(): #GP misaligned access"));
    exception(BX_GP_EXCEPTION, 0);
  }
  // BX_INFO(("PERF: TLB MISS NORMAL %lx\n", laddr));

...
void BX_CPP_AttrRegparmN(2) BX_CPU_C::VMwrite64(unsigned encoding, Bit64u val_64)
{
  ...
  switch(encoding)
  { 
    case VMCS_64BIT_GUEST_PML_CR3:
      VMCS_CACHE *vm = &BX_CPU_THIS_PTR vmcs;
      pAddr = vm->guest_pml_address + 8;
...
next:
  BX_NOTIFY_PHY_MEMORY_ACCESS(pAddr, 8, MEMTYPE(resolve_memtype(pAddr)), BX_WRITE, BX_VMX_GUEST_PML_WRITE, (Bit8u*)(&val_64));
}
...
VMX_error_code BX_CPU_C::VMenterLoadCheckVmControls(void)
{
  ...
  if (vm->vmexec_ctrls3 & VMX_VM_EXEC_CTRL3_PML_ENABLE) {
    ...
    */
    vm->guest_pml_address = VMread64(VMCS_64BIT_GUEST_PML_ADDRESS);
    if (! IsValidPageAlignedPhyAddr(vm->guest_pml_address)) {
       vm->guest_pml_address = 0UL;
    }
  }
